...
std::move(r) 的结果是一个 `const Base&&`。（需要考虑`std::move`的返回类型）
而`移动构造函数 Base(Base&&)`不接受**常量右值引用**，因此只能选择拷贝构造函数
总结：先是选择了以下重载：
```cpp
      push(value_type&& __x)
```
...
# static声明函数
## 为什么以下函数无法跨编译单元使用？就算声明和实现分离，也不行？
```cpp
...
当你在头文件中使用 static 声明函数时，这意味着该函数的链接属性为`内部链接（internal linkage）`。
每个编译单元都会将这个函数视为不同的符号，因此**每个 .cpp 文件都会有一个独立的函数**（每个编译单元内各有一份实现）。

这种情况下，每个编译单元都会有自己独立的静态局部变量，导致不同编译单元之间无法共享同一个实例。
PS: 进行汇编的时候，是不关心函数实现的，能够找到声明就足够了，链接时才必须找到实现。

## 为什么在模板类中的声明的static变量，必须在头文件中定义？
1. 模板实例化（Template Instantiation）
模板不是具体的类型，而是一个用于生成具体类型的蓝图。
  当使用模板时，编译器会根据传递给模板的参数来实例化具体的类型。这意味着，对于每个不同的模板参数组合，编译器都会生成一个新的类或函数。

如果 static 成员变量的定义放在源文件（.cpp 文件）中，那么这个定义只会为那个特定的源文件创建一个实例。而其他使用该模板的源文件将无法访问到这个 static 变量，因为它们各自会尝试创建自己的实例，导致链接错误或多个定义的问题（multiple definition errors）。
因此，`static成员变量的定义必须出现在所有使用该模板的地方都能看到的位置————即头文件中`。

2. 单一定义规则（One Definition Rule, ODR）
C++ 的单一定义规则要求在一个程序中，任何非内联的函数或变量只能有一个定义。
然而，对于模板和内联函数，ODR 有一个例外：它们可以在多个翻译单元中定义，只要这些定义是相同的。
因此，将 static 成员变量的定义放在头文件中，确保了所有使用该模板的翻译单元都能看到相同的定义，从而遵守 ODR。

3. 延迟绑定（Late Binding）
`模板的成员函数和静态成员变量通常是在第一次使用时才被实例化的，这种行为称为延迟绑定`。
如果你在源文件中定义了 static 成员变量，那么只有当那个源文件中确实使用了这个变量时，它才会被实例化。然而，如果其他源文件也使用了这个模板，但没有包含那个源文件，那么这些源文件中的代码就无法访问到这个 static 成员变量，导致链接错误。通过在头文件中定义 static 成员变量，可以确保无论何时、何地使用模板，都能正确地实例化 static 成员变量。

4. 编译依赖性
将 static 成员变量的定义放在头文件中，虽然会导致所有包含该头文件的源文件都“看到”这个定义，但这正是我们所需要的，以确保正确的实例化和链接。
不过，这也意味着更改模板的实现（包括 static 成员变量的定义）可能会触发重新编译所有包含该头文件的源文件。这是模板使用的一个常见代价，但也是一种必要的权衡，以保证模板的正确性和一致性。



## 为什么static变量必须定义一把？
***把static变量定义在类里面，就能共享，但是注意，一定要在源文件进行定义（c++11）。***
在C++11中，在类内部声明一个静态成员变量是告诉编译器这个成员属于整个类而不是类的某个特定对象。
也就是说，所有的类实例共享同一个静态成员变量。然而，**这样的声明只是告诉编译器这个静态成员的存在，并没有为它分配存储空间（``定义了才分配内存，不定义就不分配内存`）**。
根据C++的“一个定义规则”（One Definition Rule, ODR），任何非内联的函数或非内联的变量在一个程序中必须恰好有一个定义。如果静态成员变量在头文件中的类定义里直接初始化，当这个头文件被多个源文件包含时，每个源文件都会包含该变量的定义，这就违反了ODR，会导致链接错误。
总之，这样可以保证只有一份定义。

## `DATA` / `BSS`
`DATA段`
用于存储程序中的`已初始化的`全局变量和静态变量。
这些变量在程序启动时已经被赋予了初始值。
DATA段是包含在可执行文件中的，并且占用磁盘空间。
在程序运行期间，这些变量可以被修改，因此它们位于`可读写的`内存区域。

`BSS段`
用于存储程序中`未初始化的`全局变量和静态变量，它们在程序开始执行之前会被`自动初始化为零`。
这些变量在程序启动时会被自动初始化为零或空（对于指针类型）。

### 区别与特点
在初始化时 bss 段部分将会清零。bss 段属于静态内存分配，即程序一开始就将其清零了。
比如，在C语言之类的程序编译完成之后，已初始化的全局变量保存在.data 段中，未初始化的全局变量保存在.bss 段中。
text 和 data 段都在可执行文件中，由系统从可执行文件中加载；
而 bss 段不在可执行文件中（因为没有实际的值，自然是不占空间的），由系统初始化。
PS: `std::vector`这种数据都是定义在`堆`上
...
# 设计模式
> SOLID 设计原则：SOLID 是一个首字母缩写，代表以下设计原则（及其缩写）：
* 单一责任原则（SRP）
  **每个类只有一个责任，因此，只有一个改变的理由。**
    一个类只负责自己应该做的，这样子可以减少耦合（比如写日志的功能，不应该把存储文件的方法加入，如果想要持久化到云端，也要涉及到日志类的改动，牵扯太多）
* 开闭原则（OCP）
  OCP 声明你不需要返回你已经编写和测试过的代码，并对其进行更改。我们所要做的就是`实现任何一个接口（不需要修改接口本身）`来实现新的机制。
  这就是“对扩展开放，对修改封闭”的意思。

* 里氏替换原则（LSP）
  如果一个接口可以接受类型为`基类`的对象，那么它应该同样地可以接受类型为`子类`的对象，而不会有任何破坏。
  比如说有一个矩形类，和一个正方形类（派生于矩形类）
```cpp
// 正方形类
...
```
  现在有一个用于设置矩形的高度的接口，接收矩形，当把这个正方形对象传入，再对正方形调`area()`，得到的仍然是原先的面积
  实际上不应该出现一个正方形类，继承自矩形类

* 接口隔离原则（ISP）
因此，ISP 的建议是将接口分开，以便于实现者可以根据他们的需求进行挑选和选择。将复杂接口的部分分隔成单独的接口，以**避免迫使实现者实现他们并不真正需要的功能**。
这里的接口指的是抽奖类的纯虚函数。

* 依赖倒置原则（DIP）
接口或基类上的依赖比依赖于具体的类型更好。
...
类，作为一种类型定义，是没有大小可言的。
类的大小，指的是类的对象所占的大小。
因此，用sizeof对一个类型名操作，得到的是具有该类型实体的大小。

类大小的计算，遵循结构体的`对齐原则`；
类的大小，与`普通数据成员`有关，与`成员函数`和`静态成员`无关。
即普通成员函数、静态成员函数、静态数据成员、静态常量数据成员，均对类的大小无影响；
`虚函数`对类的大小有影响，是因为`虚函数表指针`带来的影响；
`虚继承`对类的大小有影响，是因`为虚基表指针`带来的影响；
静态数据成员之所以不计算在类的对象大小内，是因为类的静态数据成员被该类所有的对象所共享，并不属于具体哪个对象，静态数据成员定义在内存的全局区；
`空类`的大小（类的大小为`1`），以及含有虚函数，虚继承，多继承是特殊情况；



# 虚表的使用
`虚函数`是通过一张`虚函数表`来实现的。
编译器必需要保证`虚函数表的 指针`存在于对象实例中`最前面的位置`（这是为了保证正确取到虚函数的偏移量）。
　　每当创建一个包含有虚函数的类 或 从包含有虚函数的类派生一个类时，编译器就会为这个类创建一个虚函数表（VTABLE）保存该类所有虚函数的地址，
其实这个VTABLE的作用就是`保存自己类中所有虚函数的地址`，可以把VTABLE形象地看成一个`函数指针数组`，这个数组的每个元素存放的就是虚函数的地址。
在每个带有虚函数的类中，编译器秘密地置入一指针，称为vpointer（缩写为VPTR），指向这个对象的VTABLE。
当构造该派生类对象时，其成员VPTR被初始化指向该派生类的VTABLE。
所以可以认为：
* VTABLE是该类的所有对象共有的，在定义该类时被初始化；
...
```
Derived类对象的虚表指针所指向的虚函数表内的元素是这样子排列的：
（虚函数表内部元素的排列，在编译期就可以确定了），在这个例子里，并没有重写虚函数，所以就是正常顺位
`Base::f()`
`Base::g()`
`Base::h()`
`Derive::f1()`
...
1. 虚函数按照其声明顺序放于表中。
2. 基类的虚函数在派生类的虚函数前面。
3. 此时基类和派生类的sizeof都是数据成员的大小 + 指针的大小8。
...
`Derive:h1()`
　可以发现：
1. 覆盖的f()函数被放到了虚表中原来基类虚函数的位置；（就是很单纯地重写并覆盖了，无法通过指针调用到基类的虚函数实现了）
2. 没有被覆盖的函数依旧；
3. 派生类的大小仍是基类和派生类的非静态数据成员的大小 + 一个vptr指针的大小（8字节/64位）；

```cpp
Base *b = new Derive();
b->f();
```
由b所指的内存中的虚函数表的`f()`的位置已经被`Derive::f()`函数地址所取代，于是在实际调用发生时，是`Derive::f()`（根据实际指向的对象的第一个内存区域的虚表指针，得到虚函数表，发现虚函数进行了重写（覆盖了基类的虚函数））被调用了。
这就实现了多态。
...
一定要注意野指针问题：
```cpp
List* node;   // 错误！无法进行bool判断，一定要赋值成nullptr，这就是野指针，没有初始化，或者说指向一个随机地址
```
还要避免空悬指针，一个指向已经被释放过的内存地址的指针

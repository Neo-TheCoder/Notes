...
# 关于进程的内核空间
尽管每个进程有独立的页表，但内核在初始化和进程创建时，会确保所有进程的“内核空间”部分的页表项`指向相同的物理内存`。
内核代码和数据：内核的代码段、数据段、各种全局数据结构（如进程描述符 task_struct 的链表、文件系统结构等）在物理内存中只存在一份。
无论当前 CPU 在执行哪个进程，只要它需要执行内核函数（如系统调用），它访问的都是同一份内核代码。
内核可以方便地访问任何进程的数据，因为它可以通过自己的“内核空间”映射来访问存放 task_struct 等结构的物理内存。

而内核线程是完全在内核空间运行的“特殊进程”
...
针对高性能要求，需要多CPU并行的关键任务

# tasklet
对softirq的封装
串行执行：由同一个 tasklet 调度的函数不会在多个 CPU 上同时运行（即使在不同 CPU 上触发，也只会在一个 CPU 上串行执行），因此编程模型更简单，通常不需要额外的锁
...
# multicast threshold
linux C/C++中，SOME/IP为什么要涉及多播阈值？为什么从单播切换到多播通信可以减小负载？
* 减少`发送端`发送负载： 
    单播时需分别发送多份数据包给各订阅者，发送端CPU和socket负担高。
    多播时只需发送一份组播包，一次发送即可满足多订阅者。
* 节省网络带宽： 
    通过交换机/路由器的组播机制，`网络上`只转发`一份`数据，各接收节点从`组播组`中接收，无需多份重复数据传输。
* 提高效率，降低网络拥塞。
